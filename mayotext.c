...
void enableRawMode() {
	...
	if(tcsetattr(STDIN_FILENO, TCSAFLUSH, &raw)==-1) die("FAILED SETTING");
}
...
int editorReadKey() {
	...
	while((nread = read(STDIN_FILENO, &c, 1)) != 1) {
		if(nread == -1 && errno != EAGAIN) die("ILLITERACY");
	}
...
file to the actual file the user wants to overwrite, and they’ll carefully check
for errors through the whole process."
*/
	...
	switch(c) {
		...
		case CTRL_KEY('q'):
			if(E.dirty&&quit_times>0) {
				editorSetStatusMessage("Hey sorry you have some unsaved changes. " "Press CTRL+Q %d more times to quit! :3", quit_times);
				quit_times--;
				return;
			}
...
int main(int argc, char * argv[]) {
	...
	editorSetStatusMessage("Ctrl+S to save :} // Ctrl+Q to quit :3");
